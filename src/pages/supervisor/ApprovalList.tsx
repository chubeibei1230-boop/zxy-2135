  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold" style={{ color: 'var(--color-primary)' }}>审批列表</h2>
        <button
          onClick={() => navigate('/supervisor/export')}
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {applications.map((app) => (
              <tr key={app.id} className="border-b hover:bg-gray-50 cursor-pointer" style={{ borderColor: 'var(--color-border)' }} onClick={() => navigate(`/supervisor/applications/${app.id}`)}>
                ...
                <td className="py-2.5 text-right">
                  <button className="text-xs px-3 py-1 rounded border hover:bg-gray-50" style={{ borderColor: 'var(--color-border)', color: 'var(--color-primary)' }}>
                    审批
                  </button>
                </td>
              </tr>
            ...
            {applications.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-gray-400">暂无审批记录</td></tr>
            )}
          </tbody>
        </table>
      )}
